...
Prior to analysis, data were explored in terms of both summary statistics and visual patterns and relationships. Table 1 summarizes the data for all fourteen variables used in building the explanatory model. Summary statistics include mean, minimum value, maximum value, and standard deviation across all observations. From 2003 to 2021, Blue Mesa Reservoir's elevation ranged from 7,438 ft to 7,519 ft. Temperature at the reservoir ranged from -11.80 $^\circ$F to 87.20 $^\circ$F. The Blue Mesa dam produced an average of 18,856 MWh of electricity per month with a range of 2,724 MWh to 54,068 MWh during peak production.
...
```{r build summary statistics table, echo=FALSE}
...
From there, a few variables are explored graphically with respect to the dependent variable (MWh). In `r figr('MWHxTotal', TRUE, type = "figure")`, total inflow (cfs) is plotted against electricity generation and shows a positive relationship. As inflow increases, electricity generation increases as well. 
...
Last, `r figr('MWHxPrecip', TRUE, type = "figure")` shows precipitation (inches) plotted against electricity generation. No noticeable trend is observed but the contribution from precipitation in explaining variation in electricity generation is explored further in the analysis.
...
A multiple linear regression was then performed to explore the contribution of the thirteen independent variables in explaining variation in the dependent variable, megawatt hours (MWh). After the initial regression, an AIC test was run to identify which combination of variables contributed to the model of best fit for explaining MWh. Eight independent variables were selected for the final model and their coefficients (Beta) and significance values are summarized in Table 2. 
...
The final regression identifies elevation, storage, evaporation, inflow, maximum temperature, precipitation, total inflow, and power outflow as all contributing to explaining variation in electricity generation (MWh) across the study period. The model has an adjusted R^2 of `r r.squared`, indicating the model explains 99.5% of the variation in MWh. According to the model results, a decrease in elevation of 249 ft per month leads to an increase of one MWh of electricity generation. This result is expected considering that as water is drawn down through the hydroelectric turbines for electricity generation, the reservoir's elevation decreases. Likewise, as power outflow increases 17 cfs, electricity generation increases by one MWh per month. This result also agrees with our expected findings since power outflow and MWh are highly correlated and since power outflow is released with the intention of generating electricity.
