...
```{r load the data}
climate<-read.csv("../Project/Processed/BlueMesaClimate.csv")
power<-read.csv("../Project/Processed/BlueMesaPower.csv")
reservoir<-read.csv("../Project/Processed/BlueMesaReservoir.csv")
```
...
monthly.ts <- ts(new.04_20.data$Date, start = c(f.year, f.month), frequency=12) 
print(monthly.ts)
...
summary(MWH.monthly_SMK1)
   ##  Score =  1632 , Var(Score) = 7072, denominator =  1632, 
   ##    tau = 1, 2-sided pvalue =< 2.22e-16
...
```
>Interpretation of test results with seasonal component: Decomposing the data confirmed the presence of a seasonal trend in the data. Then the Seasonal Mann Kendall test was used to test stationarity for monotonic trends. For each season of the year represented by tau, we had a value of 1, and a p-vlaue smaller than 0.05 meaning we reject the null hypothesis and have a trend, which provides support of the alternative hypothesis. 
...
>To answer the research question, yes the MWH have steadily increased between 2004-2020 at the Blue Mesa Dam in Colorado. 
...
f.month2 <- month(first(new.04_20.data$Date))
f.year2 <- year(first(new.04_20.data$Date))
#make 2nd ts (no seasonality) for tests
MWH.monthly.ts2 <- ts(new.04_20.data$Date, start = c(f.year2, f.month2), frequency=12) 
...
#run test
MWH_monthly_SMK1.2 <- Kendall::SeasonalMannKendall(MWH.monthly.ts2)
#inspect results
G_O3_monthly_SMK1.2
   ## tau = 1, 2-sided pvalue =< 2.22e-16
...
#run 2nd test
MWH_monthly_SMK2.2 <- trend::smk.test(MWH.monthly.ts2)
#inspect results
MWH_monthly_SMK2.2

summary(MWH_monthly_SMK2.2)

```

> Answer: After removing the seasonal component, and running the Seasonal Mann Kendall test, tau stayed at a value of 1, and the p-value again was less than 0.05 giving support to reject the null hypothesis, and providing support for the alternative hypothesis. This means we do have the presence of a trend still, even without the seasonal component. 
...
#Cassidy needs help figuring out how to place the regression into an output folder alongside other folders
...
# slides to include: question, data, wrangle, exploration, analysis, 6
# mins + 1 minute of Q/A
